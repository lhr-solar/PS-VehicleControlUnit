...
static void handle_state_not_ready(void) { send_motor_drive_cmd(0.0f, 0.0f); }
...
void Task_BroadcastVCUStatus(void *args __attribute__((unused))) {
    ...
    while (1) {
        ...
        bool pedals_ok =
            !accel_brake.Accel_Pos_Main_Fault && !accel_brake.Accel_Pos_Redundant_Fault &&
            !accel_brake.Brake_Pos_Main_Fault && !accel_brake.Brake_Pos_Redundant_Fault &&
            !accel_brake.Brake_Pressure_1_Fault && !accel_brake.Brake_Pressure_2_Fault;

        bool driver_input_ok = !faults_is_active(FAULT_ID_CONTROLS_STATUS_WATCHDOG) &&
                               !faults_is_active(FAULT_ID_CONTROLS_FAULT);

        buf[1] = ((uint8_t)precharge_complete << 0) | // Motor_Contactor_State
