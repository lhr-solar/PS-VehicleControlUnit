...
void CanTxTelemetryTask_Init(void) {
    canTxTelemetryQueue =
        xQueueCreateStatic(CAN_TX_TELEMETRY_QUEUE_SIZE, sizeof(can_tx_payload_t),
                           canTxTelemetryQueueStorage, &canTxTelemetryQueueBuffer);
...
void can_fd_tx_callback_hook(FDCAN_HandleTypeDef *hfdcan, const can_tx_payload_t *payload) {
    ...
    if (canTxTelemetryQueue != NULL) {
        xQueueSendFromISR(canTxTelemetryQueue, payload, &higherPriorityTaskWoken);
    }
