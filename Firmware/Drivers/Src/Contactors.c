...
static const char *CONTACTOR_NAMES[NUM_CONTACTORS] = {"Motor Contactor", "Motor Pre Contactor"};
...
static void vContactorCallback(TimerHandle_t senseTimer) {
    ...
    if (contactor->state != contactor_get_sense(contactor_num)) {
        faults_set(FAULT_ID_CONTACTOR_CALLBACK);
    }
}

/* sets contactor, updates state value, then starts timer to check expected state matches actual
state. An error means semaphore was busy, or that I set a contactor that didn't exist. */
ErrorStatus contactor_set(contactor_num_t contactor_num, contactor_state_t state, uint32_t wait_ms,
    ...
    /* start timer to check if the state of the contactor makes expected state, the exit critical
    section. Timer resets when the contactor is set to another value, so no possible error with
    expected value changing from when timer is called*/
    if (!emergency) {
